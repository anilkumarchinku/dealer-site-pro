...
export default function UsedTwoWheelersPage() {
    ...
    const load = useCallback(async () => {
        ...
        setLoading(true)
        try {
            const res  = await fetch(`/api/two-wheelers/used?dealerId=${dealerId}&pageSize=50`)
    ...
    async function handleMarkSold(vehicle: TwoWheelerUsedVehicle) {
        ...
        const res = await fetch(`/api/two-wheelers/used/${vehicle.id}`, { method: "DELETE" })
        if (!res.ok) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold">Used 2W Stock</h1>
                    <p className="text-muted-foreground text-sm">{total} vehicle{total !== 1 ? "s" : ""} in stock</p>
                </div>
                <Button asChild>
                    <Link href="/dashboard/two-wheelers/used/add">
            ...
            {loading ? (
                ...
                <div className="text-center py-20 text-muted-foreground">
                    ...
                    <Button asChild><Link href="/dashboard/two-wheelers/used/add">Add Used Vehicle</Link></Button>
                </div>
            ) : (
