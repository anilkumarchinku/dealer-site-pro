...
export default function EditThreeWheelerVehiclePage() {
        ...
        fetch(`/api/three-wheelers/${id}`)
            ...
            .then((v: ThreeWheelerVehicle) => {
                ...
                setForm({
                    ...
                    payload_kg:              v.payload_kg?.toString() ?? "",
                    passenger_capacity:      v.passenger_capacity?.toString() ?? "",
                    permit_type:             v.permit_type ?? "city",
                    gvw_kg:                  v.gvw_kg?.toString() ?? "",
                    mileage_kmpl:            v.mileage_kmpl?.toString() ?? "",
                    cng_mileage_km_per_kg:   v.cng_mileage_km_per_kg?.toString() ?? "",
                    range_km:                v.range_km?.toString() ?? "",
                    max_speed_kmph:          v.max_speed_kmph?.toString() ?? "",
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        const payload = {
            ...
            cng_mileage_km_per_kg:   form.cng_mileage_km_per_kg   ? Number(form.cng_mileage_km_per_kg) : null,
        ...
        try {
            const res = await fetch(`/api/three-wheelers/${id}`, {
                method:  "PUT",
    ...
    return (
        <div className="max-w-2xl space-y-6">
            <div className="flex items-center gap-3">
                ...
                <h1 className="text-2xl font-bold">Edit 3W Vehicle</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <fieldset className="bg-card border border-border rounded-xl p-5 space-y-4">
                    ...
                    {images.length > 0 && (
                        <div>
                            <p className="text-xs text-muted-foreground">{images.length} image(s) attached</p>
                        </div>
                    )}
                </fieldset>
