 ...
 * Uploads a 3W vehicle image to Supabase Storage (dealer-assets bucket)
...
import { requireAuth, getDealerForUser, createAdminClient } from '@/lib/supabase-server'
...
export async function POST(request: NextRequest) {
    ...
    // Build unique storage path: thw/{dealerId}/{timestamp}-{random}.{ext}
    const ext         = file.name.split('.').pop()?.toLowerCase() ?? 'jpg'
    const safeName    = `${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`
    const storagePath = `thw/${dealer.id}/${safeName}`
    ...
    if (uploadError) {
        console.error('[thw/upload-image] Storage upload error:', uploadError)
