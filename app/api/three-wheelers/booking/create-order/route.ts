...
export async function POST(request: NextRequest) {
    const rateLimit = await rateLimitOrNull('thw_booking_create', request, 10, 60 * 60 * 1000)
    ...
    const bookingResult = await createThreeWheelerBooking({
        dealer_id,
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error('[thw_booking] Razorpay order error:', data)
        ...
        console.error('[thw_booking] Razorpay fetch error:', error)
