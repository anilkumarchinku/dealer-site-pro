...
function verifyRazorpaySignature(orderId: string, paymentId: string, signature: string): boolean {
    ...
    if (!secret || secret === 'your_razorpay_secret_key_here') {
        if (process.env.NODE_ENV === 'production') {
            console.error('[thw_booking/verify] RAZORPAY_KEY_SECRET not configured — rejecting')
            return false
        }
        console.warn('[thw_booking/verify] Skipping signature check in dev')
        return true
    }
...
export async function POST(request: NextRequest) {
    ...
    if (!isValid) {
        await updateThreeWheelerBookingPayment(idempotencyKey, orderId, paymentId, 'failed')
        return NextResponse.json({ success: false, error: 'Invalid payment signature' }, { status: 400 })
    }

    // ── Update booking to paid ──────────────────────────────────
    const result = await updateThreeWheelerBookingPayment(idempotencyKey, orderId, paymentId, 'paid')
    if (!result.success) {
