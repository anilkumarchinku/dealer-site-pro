...
export default function CargoThreeWheelerPage() {
    ...
    useEffect(() => {
        ...
        if (filters.minPayload) p.set("minPayload", String(filters.minPayload))
        if (filters.maxPayload) p.set("maxPayload", String(filters.maxPayload))
    ...
    return (
        <div className="min-h-screen max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="mb-8">
                <h1 className="text-3xl font-bold">📦 Cargo 3-Wheelers</h1>
                <p className="text-muted-foreground mt-1">{total} model{total !== 1 ? "s" : ""} available — Piaggio Ape, Mahindra Alfa &amp; more</p>
            </div>
            ...
            <div className="flex gap-8">
                ...
                <div className="w-52 shrink-0 hidden md:block">
                    <FilterSidebar filters={filters} onChange={setFilters} brands={brands} showCargo={true} />
                </div>
                ...
                <div className="flex-1">
                    {loading ? (
                        ...
                        <div className="text-center py-20 text-muted-foreground">
                            ...
                            <p className="text-lg font-medium mt-4">{search ? "No results found" : "No cargo vehicles available right now"}</p>
                            <p className="text-sm mt-1">{search ? "Try a different search term." : "Contact us for custom requirements or fleet orders."}</p>
                            <button onClick={() => { setFilters({ type: "cargo", sortBy: "newest", page: 1, pageSize: 12 }); setSearch("") }} className="mt-3 text-sm text-primary hover:underline">Clear filters</button>
                        </div>
                    ) : (
                        <>
                            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
