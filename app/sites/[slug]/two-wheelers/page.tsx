...
export default function TwoWheelersHomePage() {
    ...
    const categories = [
        { label: "Bikes",    href: `/sites/${slug}/two-wheelers/bikes`,    emoji: "🏍️" },
        { label: "Scooters", href: `/sites/${slug}/two-wheelers/scooters`, emoji: "🛵" },
        { label: "Electric", href: `/sites/${slug}/two-wheelers/electric`, emoji: "⚡" },
        { label: "Used",     href: `/sites/${slug}/two-wheelers/used`,     emoji: "🔄" },
    ]
        ...
        <div className="min-h-screen">
            ...
            <section className="bg-gradient-to-br from-primary/10 via-background to-background py-16 px-4">
                <div className="max-w-5xl mx-auto text-center space-y-4">
                    ...
                    <p className="text-lg text-muted-foreground">Your trusted 2-Wheeler destination in {dealer.location}</p>
                    <div className="flex flex-wrap justify-center gap-3 pt-2">
                        <Link href={`/sites/${slug}/two-wheelers/bikes`} className="bg-primary text-primary-foreground rounded-lg px-5 py-2.5 font-medium hover:opacity-90">Browse Bikes</Link>
                        <Link href={`/sites/${slug}/two-wheelers/service`} className="border border-border rounded-lg px-5 py-2.5 font-medium hover:bg-muted/50">Book Service</Link>
                        <Link href={`/sites/${slug}/two-wheelers/emi-calculator`} className="border border-border rounded-lg px-5 py-2.5 font-medium hover:bg-muted/50">EMI Calculator</Link>
                    </div>
                </div>
            </section>
            ...
            {featured.length > 0 && (
                <section className="max-w-5xl mx-auto px-4 py-10">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Link href={`/sites/${slug}/two-wheelers/bikes`} className="text-sm text-primary hover:underline">View all →</Link>
                    </div>
            ...
            <section className="max-w-5xl mx-auto px-4 py-10">
                <div className="grid sm:grid-cols-3 gap-4">
                    <Link href={`/sites/${slug}/two-wheelers/compare`} className="p-4 border border-border rounded-xl hover:border-primary/40 transition-colors">
                    ...
                    <Link href={`/sites/${slug}/two-wheelers/offers`} className="p-4 border border-border rounded-xl hover:border-primary/40 transition-colors">
                        <p className="font-semibold">🎁 Offers</p>
                        <p className="text-sm text-muted-foreground mt-1">Current schemes and discounts</p>
                    </Link>
                    <Link href={`/sites/${slug}/two-wheelers/emi-calculator`} className="p-4 border border-border rounded-xl hover:border-primary/40 transition-colors">
