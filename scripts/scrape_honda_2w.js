/**
 * scrape_honda_2w.js
 * Scrapes high-quality images for Honda 2W models.
 * Saves them to public/data/brand-model-images/2w/honda/
 */
...
const HONDA_DATA = path.join(process.cwd(), 'public/data/2w/honda.json');
const DEST_DIR = path.join(process.cwd(), 'public/data/brand-model-images/2w/honda');
...
async function scrapeImage(page, model) {
    // Specific query for Honda bikes to get clean side views
    const query = `Honda ${model} bike side view 2024 white background`;
...
async function main() {
    ...
    const data = JSON.parse(fs.readFileSync(HONDA_DATA, 'utf8'));
    const models = data.vehicles;

    console.log(`🚀 Starting Honda 2W image scrape — ${models.length} models\n`);
    ...
    for (const v of models) {
        ...
        await delay(1500); // Slightly more delay for more models
    }
