...
const SERVICE_TYPES: { value: ThreeWheelerServiceType; label: string }[] = [
    { value: "general_service", label: "General Service"  },
    { value: "oil_change",      label: "Oil Change"       },
    { value: "tyre",            label: "Tyre Service"     },
    { value: "battery",         label: "Battery Service"  },
    { value: "cng_kit",         label: "CNG Kit Service"  },
...
export function ServiceBookingForm({ dealerId }: Props) {
    const [form, setForm] = useState({
        ...
        vehicle_reg_no:  "",
        km_reading:      "",
        service_type:    "" as ThreeWheelerServiceType | "",
        preferred_date:  "",
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input value={form.vehicle_make} onChange={e => set("vehicle_make", e.target.value)} className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="e.g. Bajaj" />
                </div>
                <div>
                    <label className="text-sm font-medium">Vehicle Model</label>
                    <input value={form.vehicle_model} onChange={e => set("vehicle_model", e.target.value)} className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="e.g. RE Compact" />
                </div>
            </div>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input value={form.vehicle_year} onChange={e => set("vehicle_year", e.target.value)} type="number" min={1990} max={2030} className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="2022" />
                </div>
                <div>
                    <label className="text-sm font-medium">Reg. Number</label>
                    <input value={form.vehicle_reg_no} onChange={e => set("vehicle_reg_no", e.target.value)} className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="MH12AB1234" />
                </div>
                <div>
                    <label className="text-sm font-medium">KM Reading</label>
                    <input value={form.km_reading} onChange={e => set("km_reading", e.target.value)} type="number" className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="50000" />
                </div>
            </div>
